    ...
    for i in range(2, row - 1):
        for j in range(2, col - 1):
            if (image[i][j] == 1):
                ...
                compute = (p1 * p9 * p2) + (p1 * p9 * p8) + (p1 * p8 * p7) + (p1 * p7 * p6) + (p1 * p6 * p5) + (
                            p1 * p5 * p4) + (p1 * p4 * p3) + (p1 * p3 * p2)
    ...
    while changing1 or changing2:
        ...
        for x in range(1, rows - 1):
            for y in range(1, columns - 1):
                ...
                if (Image_Thinned[x][y] == 1 and 2 <= sum(n) <= 6 and transitions(
                        n) == 1 and P2 * P4 * P6 == 0 and P4 * P6 * P8 == 0):
        ...
        for x in range(1, rows - 1):
            for y in range(1, columns - 1):
                P2, P3, P4, P5, P6, P7, P8, P9 = n = neighbours(x, y, Image_Thinned)
                if (Image_Thinned[x][y] == 1 and 2 <= sum(n) <= 6 and transitions(
                        n) == 1 and P2 * P4 * P8 == 0 and P2 * P6 * P8 == 0):
...
if __name__ == "__main__":
    # Locate a road pixel on the map
    ...
    # Change all road pixels to white and other pixels to black
    picture = Image.open("mall3.jpg")
    r, g, b = picture.getpixel((a[0], b[0]))
    print("Red: {0}, Green: {1}, Blue: {2}".format(r, g, b))

    pixels = picture.load()  # create the pixel map

    for i in range(picture.size[0]):  # for every pixel:
        for j in range(picture.size[1]):
            x, y, z = picture.getpixel((i, j))
            if math.sqrt((x - r) ** 2 + (y - g) ** 2 + (z - b) ** 2) < 10:
    ...
    plt.imshow(picture)
    plt.show()
    picture = picture.save("blackAndWhite.png")

    # thinning the map "blackAndWhite"
    test_path = "num"
    count = 0
    reduction_rate = 0
    sensitivity = 0
    ...
    for file in glob.glob("blackAndWhite.png"):
        count = count + 1
        ...
        plt.imsave('result.png', skeleton, cmap=plt.cm.gray)
